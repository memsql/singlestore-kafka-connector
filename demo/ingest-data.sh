...
do
  docker run \
    ...
    bash -c 'cat /opt/songs.json | kafka-console-producer --request-required-acks 1 --broker-list kafka-short-demo:9092 --topic singlestore_json_songs --producer-property interceptor.classes=io.confluent.monitoring.clients.interceptor.MonitoringProducerInterceptor --producer-property acks=1' >/dev/null 2>/dev/null
