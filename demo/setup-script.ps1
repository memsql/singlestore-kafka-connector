...
docker cp singlestore-kafka-connect-short-demo:/home/app/target/singlestore-kafka-connector-1.2.8.jar "$env:TEMP"
docker stop singlestore-kafka-connect-short-demo > $null
Write-Output "Success!"
...
function Start-Kafka-Connect {
    ...
    $kafkaConnectorVolumes = $nixTempPath + "/singlestore-kafka-connector-1.2.8.jar:/usr/share/java/singlestore-kafka-connector-1.2.8.jar"
